...
int main(int argc, char **argv){
    ...
    for(t = 0; t < max_time-1; t++){
        ...
        for(p = 1; p < cols_per_proc - 1; p++){
            // printf("tp: %d %d --->", t, p);
            // printf("lr - %f %f\n", proc_data[t][p-1], proc_data[t][p+1]);
    ...
    MPI_Gather(proc_data, cols_per_proc, MPI_DOUBLE,
                ...
                root_processor, MPI_COMM_WORLD);  // take another look at gather --> inappropiate use of H matrix most likely
    // try rearrange H so it's a big block of matrix, readjust gather style; interleave the
    // gather 0th row, 1st row; loop of gathers
